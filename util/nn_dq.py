...
class DeepQLearner:
    ...
    def build_network(self, network_type, input_width, input_height,
        ...
        if network_type == "nature_cuda":
            ...
            return self.build_simple_network(input_width, input_height, output_dim, num_frames, batch_size)
    ...
    def train(self, memory):
        ...
        print "training loss {}".format(np.sqrt(loss))
        return np.sqrt(loss)
    ...
    def build_simple_network(self, input_width, input_height, output_dim, num_frames, batch_size):
        """
        ...
        # third hidden layer
        l_h3 = lasagne.layers.Conv2DLayer(
            l_h2, num_filters=64, filter_size=(3, 3), stride=(1, 1),
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform())
        
        # fourth hidden layer
        l_h4 = lasagne.layers.DenseLayer(
            l_h3, num_units=512, 
            nonlinearity=lasagne.nonlinearities.rectify)

        # output layer: linearity must be explicit
        l_out = lasagne.layers.DenseLayer(
            l_h4, num_units=output_dim,
            nonlinearity=None)
    ...
    def get_settings(self):
        ...
        settings = {
            "gamma": self.discount,
            "target_copy_interval": self.freeze_interval,
            "network": network,
        }
